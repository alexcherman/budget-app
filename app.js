...
var UIController = (function() {
	...
	var formatNumber = function(num, type) {
		...
		num = Math.abs(num); // Remove the sign of the number and overriding num variable
		num = num.toFixed(2); // Put two decimals on the number

		numSplit = num.split('.') // Separate number to decimal part and integer part

		int = numSplit[0]; // Integers
		if (int.length > 3) {
	...
	return { getInput: function() {
      return { type: document.querySelector(DOMstrings.inputType).value, description: document.querySelector(DOMstrings.inputDescription).value, value: parseFloat(document.querySelector(DOMstrings.inputValue).value) }; // Will be either inc or exp
